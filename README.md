...
| Header          | `contacts`, `contactsep`, `contactbreak`, `headerlayout`, `headerimage`, `headerimagesize`, `headerimagegap`, `headerimagepagegap`, `headerimagetextgap`, `headerimageposition` | Use `\cvcontact{icon}{text}[link]` per item. Set `headerlayout = {left}` to align text left; `headerimageposition = {page}` pins the optional circular image to the top-right edge. `headerimagegap` updates both dedicated gap knobs at once. |
...
- `headerimage`, `headerimagesize`, `headerimagegap`, `headerimagepagegap`, `headerimagetextgap`, `headerlayout`, and `headerimageposition` let you drop a circular profile photo in the top-right corner. `headerimagepagegap` nudges the image in from the page edge, while `headerimagetextgap` trims space from the text column. Switch `headerimageposition` to `flow` if you prefer it inline with the name.
